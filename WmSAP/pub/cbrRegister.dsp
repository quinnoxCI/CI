<html>
  ...
  <body onLoad="setNavigation('/WmSAP/cbr.dsp?adapterTypeName=WmSAP', '/WmSAP/doc/OnlineHelp/wwhelp/wwhimpl/js/html/wwhelp.htm?context=Help&topic=IS_RG_SVC', 'foo');">
	...
	<td class="menusection-Adapters" colspan="2">Adapters &gt;
	  SAP Adapter &gt; %ifvar action equals('Unregister')%Unregister%else%Register%endif%
	...
	<td>
	  <form action="cbr.dsp" method="post">
	    ...
	    <table class="tableForm" width="95%">
	      <tr>
		<td class="heading" colspan="2">%ifvar action equals('Unregister')%Unregister%else%Register%endif%
